...
`ca_spawn()` records one request. A worker calls `ca_claim()` or `ca_work()` to
pull eligible work. In a handler, `ca_step()` returns a saved value when that
step completed earlier; otherwise it runs the callback and saves its value.
The task reaches `completed`, `failed`, or `cancelled` when no further worker
