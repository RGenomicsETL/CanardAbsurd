...
CanardAbsurd records queued R jobs in DuckDB. A worker claims a task, saves the
result of each named step, and another worker can continue after the first one
exits.

For a database shared by more than one process, run a Quack server. It is the
only process that opens the database file; producers and workers connect to it
through Quack. This prevents competing file access and DuckDB/WAL locking.

A worker can finish an external action and exit before saving its checkpoint.
Give each external action an idempotency key so that a later attempt can safely
repeat it.
...
requires an explicitly installed DuckDB Quack extension; follow the
...
for failure budgets, cancellation, and operational limits.
[Native values and DuckDB R compatibility](https://rgenomicsetl.github.io/CanardAbsurd/articles/native-values.html)
documents the R mappings, nested-NULL driver issue, indexed-VARIANT engine issue,
and the typed reads used by claims and replay. Payloads have no package-imposed
byte quotas; engine and transport limits still apply.
[Serialized R objects in BLOBs](https://rgenomicsetl.github.io/CanardAbsurd/articles/serialized-values.html)
shows explicit base R, qs2 and Sakura paths for values outside the native mapping.
