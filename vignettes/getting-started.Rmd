...
Only the task, checkpoint, and result are durable. The handler lives in a
worker process. Each claim creates an attempt and grants a lease. A named step
links `ca_step()` to its checkpoint; a later attempt receives that checkpoint
without running its callback again. A stale or cancelled worker cannot write.
