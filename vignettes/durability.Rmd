...
and results use JSON text rather than opaque R serialization so their contents
remain visible to SQL. Checkpoint names and kinds are available through
`canard_absurd.task_checkpoints`; its `value_json` column holds the encoded step
value.
...
")
DBI::dbGetQuery(db@con, "
  SELECT name, kind, value_json
  FROM canard_absurd.task_checkpoints
  WHERE task_id = ?
", params = list(id))
```

These queries need no DuckDB extensions. Analysts who explicitly install and
load the JSON extension can cast `input`, `result`, or `value_json` to `JSON` for
nested field extraction. CanardAbsurd does not load or install that extension.
...
- Schema version 1 limits input to 1 MiB of UTF-8 JSON and each task's checkpoint
  map to 16 MiB. Results and error text have no additional package-imposed size
  cap.
